...
class Together_Embedder(Embedder):
  ...
  #interface method
  def generate_vectorList_as_floatLists_from_URL(self, file_url: str) -> list[list[float]]: #TODO testing required
  ...
  def generate_vectorStore_from_URL(self, file_url: str, file_extension: str = ".") -> InMemoryVectorStore:
    ...
    vectorStore = self.generate_vectorStore_from_file(self, downloaded_file_path)
  ...
  def elaborate_most_suitable_sentences(self, query: str, vectorStore: InMemoryVectorStore) -> list[str]:
    """
    ...
    return self.elaborate_most_suitable_sentences(query, retriever)
  
  #TODO consider to add an output limit option (for now seems to be 4 by default)
  def elaborate_most_suitable_sentences(self, query: str, retriever: VectorStoreRetriever) -> list[str]:
    """
...
if __name__ == "__main__":
  ...
  #execute test
  t_RAG = Together_Embedder(model_name, together_API_key)
  vectorStore = t_RAG.generate_vectorStore_from_clusteredText(textList)
  retrieved_sentences = t_RAG.elaborate_most_suitable_sentences(query, vectorStore)
  vectorList = t_RAG.generate_vectorList_as_floatLists_from_URL(vectorStore)
